...
int Keypad_IsAnyKeyPressed(void) {
	...
	if ( debounce_state == 0 ) {						//
		...
		if ((KEYPAD_PORT->IDR & ROW_IO) != 0 ) {        // got a keypress!
			for ( uint16_t idx=0; idx<SETTLE; idx++ )   // let btn settle
				;	//do nothing
			if ((KEYPAD_PORT->IDR & ROW_IO) != 0 ) {	// still read a press
				debounce_state = 1;
				return( 1 );
			}
		}
	...
	if (debounce_state == 1 ) {							// Key was pressed
		KEYPAD_PORT->BSRR = COL_IO;         	      	// set all columns HI
		for ( uint16_t idx=0; idx<SETTLE; idx++ )   	// let it settle
			;
		if ((KEYPAD_PORT->IDR & ROW_IO) == 0 ) {		// key unpressed!
			for ( uint16_t idx=0; idx<SETTLE; idx++ )   // let btn settle
				;	//do nothing
			if ((KEYPAD_PORT->IDR & ROW_IO) == 0 ) {	//still unpressed key
				debounce_state = 0;
				return( 0 );
			}
		}
